...
uint8 setTMCLStatus(uint8 evalError)
{
	if(evalError == ERROR_NONE)          ActualReply.Status = REPLY_OK;
	else if(evalError & ERROR_FUNCTION)  ActualReply.Status = REPLY_INVALID_CMD;
	else if(evalError & ERROR_TYPE)      ActualReply.Status = REPLY_INVALID_TYPE;
	else if(evalError & ERROR_MOTOR)     ActualReply.Status = REPLY_INVALID_TYPE; // todo CHECK ADD 2: Different errors for Evalboard type/motor errors? (LH) #1
	else if(evalError & ERROR_VALUE)     ActualReply.Status = REPLY_INVALID_VALUE;
	else if(evalError & ERROR_NOT_DONE)  ActualReply.Status = REPLY_DELAYED;
	return evalError;
}
...
void ExecuteActualCommand()
{
	...
	case TMCL_ROR:
		...
		if(setTMCLStatus(Evalboards.ch1.right(ActualCommand.Motor, ActualCommand.Value.Int32)) & ERROR_FUNCTION)
		{
	...
	case TMCL_ROL:
		// if function doesn't exist for ch1 try ch2
		if(setTMCLStatus(Evalboards.ch1.left(ActualCommand.Motor, ActualCommand.Value.Int32)) & ERROR_FUNCTION)
		{
	...
	case TMCL_MST:
		// if function doesn't exist for ch1 try ch2
		if(setTMCLStatus(Evalboards.ch1.stop(ActualCommand.Motor)) & ERROR_FUNCTION)
		{
	...
	case TMCL_MVP:
		...
		case MVP_ABS: // move absolute
			if(setTMCLStatus(Evalboards.ch1.moveTo(ActualCommand.Motor, ActualCommand.Value.Int32)) & ERROR_FUNCTION)
			{
				setTMCLStatus(Evalboards.ch2.moveTo(ActualCommand.Motor, ActualCommand.Value.Int32));
			}
			break;
		case MVP_REL: // move relative
			if(setTMCLStatus(Evalboards.ch1.moveBy(ActualCommand.Motor, &ActualCommand.Value.Int32)) & ERROR_FUNCTION)
			{
			...
			break;
		case MVP_PRF:
			if(setTMCLStatus(Evalboards.ch1.moveProfile(ActualCommand.Motor, ActualCommand.Value.Int32)) & ERROR_FUNCTION)
			{
	...
	case TMCL_SAP:
		// if function doesn't exist for ch1 try ch2
		if(setTMCLStatus(Evalboards.ch1.SAP(ActualCommand.Type, ActualCommand.Motor, ActualCommand.Value.Int32)) & (ERROR_TYPE | ERROR_FUNCTION))
		{
	...
	case TMCL_GAP:
		// if function doesn't exist for ch1 try ch2
		if(setTMCLStatus(Evalboards.ch1.GAP(ActualCommand.Type, ActualCommand.Motor, &ActualReply.Value.Int32)) & (ERROR_TYPE | ERROR_FUNCTION))
		{
	...
	case TMCL_UF4:
		// if function doesn't exist for ch1 try ch2
		if(setTMCLStatus(Evalboards.ch1.getMeasuredSpeed(ActualCommand.Motor, &ActualReply.Value.Int32)) & ERROR_FUNCTION)
		{
	...
	case TMCL_UF5:
		...
		Evalboards.ch1.writeRegister(ActualCommand.Type, ActualCommand.Value.Int32);
		break;
	case TMCL_UF6:
		...
		Evalboards.ch1.readRegister(ActualCommand.Type, &ActualReply.Value.Int32);
		break;
	...
	case TMCL_UF_CH1:
		...
		Evalboards.ch1.userFunction(ActualCommand.Type, ActualCommand.Motor, &ActualCommand.Value.Int32);
		ActualReply.Value.Int32 = ActualCommand.Value.Int32;
		break;
	case TMCL_UF_CH2:
		// user function for driver board
		Evalboards.ch2.userFunction(ActualCommand.Type, ActualCommand.Motor, &ActualCommand.Value.Int32);
		ActualReply.Value.Int32 = ActualCommand.Value.Int32;
		break;
	...
	case TMCL_writeRegisterChannel_2:
		Evalboards.ch2.writeRegister(ActualCommand.Type, ActualCommand.Value.Int32);
		break;
	case TMCL_readRegisterChannel_1:
		Evalboards.ch1.readRegister(ActualCommand.Type, &ActualReply.Value.Int32);
		break;
	case TMCL_readRegisterChannel_2:
		Evalboards.ch2.readRegister(ActualCommand.Type, &ActualReply.Value.Int32);
		break;
...
static void HandleRamDebug()
{
	...
	case 0:
		// get ram buffer size
		ActualReply.Value.Int32 = debug_getRamBufferSize();
		break;
	case 1:
		// number of elements
		ActualReply.Value.Int32 = debug_getRamBufferElements();
		break;
	case 2:
		// entries per ram buffer element
		ActualReply.Value.Int32 = debug_getRamBufferElementItems();
		break;
	case 3:
		// value of an element's item
		tmp = debug_getRamBufferItem(ActualCommand.Value.Int32, ActualCommand.Motor, (int *) &ActualReply.Value.Int32);

		if(tmp == 1)
			ActualReply.Status = REPLY_INVALID_VALUE;
		else if(tmp == 2)
			ActualReply.Status = REPLY_INVALID_TYPE; // todo CHECK ADD 2: Different errors for Evalboard type/motor errors? (LH) #2
		break;
	case 4:
		// start debugging
		debug_startRAMDebugging(ActualCommand.Motor, ActualCommand.Value.Int32);
		break;
	case 5:
		// stop debugging
	...
	case 6:
		// still running?
		ActualReply.Value.Int32 = debug_isRamDebuggingEnabled() | debug_isWaitingForTrigger();
		break;
	case 7:
//		debug_setRamMeasureMode(ActualCommand.Value.Int32);
		break;
	case 8:
//		ActualReply.Value.Int32 = debug_getRamMeasureMode();
		break;
	case 9:
	//	debug_setRamMeasureDelay(ActualCommand.Value.Int32);
		break;
	case 10:
//		ActualReply.Value.Int32 = debug_getRamMeasureDelay();
		break;
	default:
		ActualReply.Status = REPLY_INVALID_TYPE;
		break;
